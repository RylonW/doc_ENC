...
def main():
    ...
    parser.add_argument('--opt', type=str, default='./options/train/test_Enhance_MSEC.yml',
    ...
    para = {}
    for k,v in torch.load('/home/wrl/doc-ENC-1.0/pretrain/350_G.pth').items():
        k = 'module.'+k
        para[k] = v
    model.netG.load_state_dict(para, strict=True)
    logger.info('Successfully load the parameters')
    ###################
    ...
    # # # #
        ...
        if opt['datasets'].get('val', None) and epoch % opt['train']['val_epoch'] == 0:
            ...
            for val_data in val_loader:
                ...
                print(real_name)
                target_root = '/home/wrl/doc-ENC-1.0/output/experiments/STEN/test_images/'
                if(not os.path.exists(target_root)):
                    os.mkdir(target_root)
                cv2.imwrite(target_root+real_name, en_img)
                ############################
            ...
            logger.info('# Validation # PSNR: Exp1 {:.4e}, Exp2 {:.4e}, Exp3 {:.4e}, Exp4 {:.4e}, Exp5 {:.4e}, '.
                        format(avg_psnr_exp1 / 150.0, avg_psnr_exp2 / 150.0, avg_psnr_exp3 / 150.0, avg_psnr_exp4 / 150.0,
                               avg_psnr_exp5 / 150.0))
            logger.info(
